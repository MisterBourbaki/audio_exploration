    ...
    for i in range(len(hidden_dims) - 1):
        _layers.append(
            nn.Sequential(
                ...
                nn.InstanceNorm1d(hidden_dims[i + 1]),
                nn.LeakyReLU(),
            )
        )
...
def build_full_decoder1d(
    ...
    module = nn.Sequential(
        ...
        create_final_layer1d(last_dim=hidden_dims[-1]),
    )
...
def create_final_layer1d(
    ...
    mod = nn.Sequential(
        ...
        nn.InstanceNorm1d(last_dim),
        nn.LeakyReLU(),
        nn.Conv1d(last_dim, out_channels=3, kernel_size=3, padding=1),
        nn.Tanh(),
    )
